...
function calcValue(card) {
    let v = card[0];
    if (v == 'A') { return 1; } else if (['K', 'Q', 'J', '1'].includes(v)) { return 10; } else { return parseInt(v); }
}
...
function getBetOutCome(_bet) {
    var bet = _bet.toLowerCase()
    if (bet == "player") {
        return "P"
    }
    if (bet == "banker") {
